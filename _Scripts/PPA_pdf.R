...
sheets <- readxl::excel_sheets(paste0(root,"_Inputs/PDF.xlsx"))
sheets <- sheets[[2]]
## ------------------------------------------------------- #
PDF_scrapp <- function(sheets){
  tryCatch({
    ...
    for(i in 1:length(links)){
      ...
      out_words <- source(paste0(root,"_Inputs/outwords.R"),encoding="utf-8")
