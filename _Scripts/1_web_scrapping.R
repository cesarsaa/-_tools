...
sheets <- readxl::excel_sheets(paste0(root,"_Inputs/WEB.xlsx"))
## ------------------------------------------------------- #
web_scrapp <- function(sheets) {
  tryCatch({
    # 
    x_files <- readxl::read_excel(paste0(root,"_Inputs/WEB.xlsx"), sheet = sheets)
    #
    ...
    for( i in 1:length(nodes)){
      ...
      #= -------------------------------------------------------
      source("D:/OneDrive - CGIAR/Documents/GitHub/XX_tool/_Inputs/outwords_2.R",encoding="utf-8")
      ...
      largas <- db[nchar(db$word)>13,] 
      db <- db %>% filter(!word %in% cortas) %>% filter(!word %in% largas)
      bigrams_separated <- bigrams_separated %>% 
        filter(!word1 %in% cortas) %>% filter(!word1 %in% largas) %>% 
        filter(!word2 %in% cortas) %>% filter(!word2 %in% largas)
      #= -------------------------------------------------------
    ...
    web_data_idf<- web_data %>% 
      tidytext::bind_tf_idf(word, category, n) %>%
      plyr::arrange(desc(tf_idf))
    #
    write.csv(web_data, paste0("D:/OneDrive - CGIAR/Documents/GitHub/XX_tool/_Results/_WEBS/web_scrapping_",sheets,"_",Sys.Date(),".csv"),quote = T)
    write.csv(web_iki_bigram, paste0("D:/OneDrive - CGIAR/Documents/GitHub/XX_tool/_Results/_WEBS/web_bigram_scrapping_",sheets,"_",Sys.Date(),".csv"),quote = T)
    write.csv(web_data_idf, paste0("D:/OneDrive - CGIAR/Documents/GitHub/XX_tool/_Results/_WEBS/web_idf_scrapping_",sheets,"_",Sys.Date(),".csv"),quote = T)
